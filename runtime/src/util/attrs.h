...
#ifdef __GNUC__
#define IEO_GCC_GTEQ(x, y)                                                     \
  (__GNUC__ > (x) || __GNUC__ == (x) && __GNUC_MINOR__ >= (y))
#define IEO_GCC_LTEQ(x, y)                                                     \
  (__GNUC__ < (x) || __GNUC__ == (x) && __GNUC_MINOR__ <= (y))
#else
 ...
 * functions declared inline, this attribute inlines the function independent of
 * any restrictions that otherwise apply to inlining. Failure to inline such a
 * function is diagnosed as an error. Note that if such a function is called
 * indirectly the compiler may or may not inline it depending on optimization
 ...
 * depends only on the parameters and/or global variables. Calls to such
 * functions can be subject to common subexpression elimination and loop
 ...
 * defintion than the const attribute: it allows the function to read global
 * variables. Decorating the same function with both the pure and the const
 * attribute is diagnosed. Because a pure function cannot have any side effects
 ...
 * effects except to return a value. Calls to such functions lend themselves to
 ...
 * attribute below because it prohibits the function from reading global
 * variables. Consequently, the presence of the attribute on a function
 * declaration allows GCC to emit more efficient code for some calls to the
 * function. Decorating the same function with both the const and the pure
 ...
 * function usually must not be const. Because a const function cannot have any
 * side effects it does not make sense for such a function to return void.
 ...
 * more aggressively and on many targets it is placed into a special subsection
 * of the text section so all hot functions appear close together, improving
 * locality.
 */
...
/**
 * @def IEO_DEPRECATED
 * The deprecated attribute results in a warning if the item is used anywhere in
 * the source file. This is useful when identifying items that are expected to
 * be removed in a future version of a program. The warning also includes the
 * location of the declaration of the deprecated item, to enable users to easily
 * find further information about why the item is deprecated, or what they
 * should do instead.
 */
 ...
 * have this property, as they can return a pointer to storage containing
 * pointers.
 */
#if defined(__GNUC__) || defined(__clang__)
...
/**
 * @def IEO_NULLABLE
 * The variable/parameter may be initialized with NULL, or the function may
 * return NULL.
 */
#define IEO_NULLABLE
...
/**
 * @def IEO_NULL
 * The variable/parameter must always be initialized with NULL (this can change
 * during function evaluation later), or function always return NULL.
 */
#define IEO_NULL
